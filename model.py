...
for line in lines:
  for i in range(3):                            # using multiple cameras
    source_path=line[i]
    #print(source_path)
    ...
    if (i==1):
      measurements.append(measurement + 0.05)   # correcting for steering from the left camera image
    elif (i==2):
      measurements.append(measurement - 0.05)   # correcting for steering from the right camera image
    else:
      measurements.append(measurement)


for line in lines1:
  for i in range(3):                            # using multiple cameras from the additional data
    source_path=line[i]
    #print(source_path)
    filename = source_path.split('/')[-1]
